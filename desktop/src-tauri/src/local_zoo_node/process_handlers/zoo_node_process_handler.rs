...
impl ZooNodeProcessHandler {
    ...
    pub async fn spawn(&self) -> Result<(), String> {
        ...
        // Coexist with hanzo-node on this host: the node binary reads NODE_API_HTTPS_PORT
        // (not NODE_HTTPS_PORT) and NODE_ZAP_PORT, both of which otherwise default to
        // hanzo's 9553/3692 and collide. Identity ending must be a valid one (.sep-hanzo).
        env.insert("NODE_API_HTTPS_PORT".to_string(), "2003".to_string());
        env.insert("NODE_ZAP_PORT".to_string(), "2004".to_string());
        env.entry("GLOBAL_IDENTITY_NAME".to_string())
            .or_insert_with(|| "@@localhost.sep-hanzo".to_string());
        // The default EMBEDDINGS_SERVER_URL ("zoo-backend:FREE_TEXT_INFERENCE") is not a
        // reachable URL and stalls node init; point at the local ollama (shared on :11435).
        env.insert(
