    ...
    pub async fn kill(&self) {
        ...
        if let Some(child) = process.take() {
            ...
            *process = None;
            ...
            // Not an error: killing when nothing is running is a no-op (happens
            // on the startup cleanup pass before any node has been spawned).
            log::debug!("[{}] no process is running to kill", self.process_name);
        }
    }
}
